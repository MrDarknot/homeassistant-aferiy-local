...
class AferiyP280Coordinator(
    ...
    async def _find_device(self):
        ...
        raise UpdateFailed(
            f"P280 not reachable: {self.address}"
        )
    ...
    async def _async_update_data(
        ...
        try:
            ...
            client = await establish_connection(
                BleakClientWithServiceCache,
                ble_device,
                ble_device.name or "AFERIY P280",
                max_attempts=3,
            )
            ...
            raise UpdateFailed(
                "Timed out waiting for P280 status"
            ) from err

        except BleakError as err:

            raise UpdateFailed(
                f"Bluetooth error: {type(err).__name__}: {err}"
            ) from err

        except ValueError as err:

            raise UpdateFailed(
                f"Invalid P280 data: {err}"
            ) from err
